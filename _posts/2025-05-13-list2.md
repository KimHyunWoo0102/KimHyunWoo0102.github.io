...
다양한 형식의 리스트를 만들게 하기 위해서 인터페이스를 정의한다.

templete을 활용하여 다양한 데이터를 저장하며 처리하게끔 한다.
...
try {
    if (!_list.isEmpty())
        throw InitializationFailureException(__func__,
                                             "List is not empty on initialization.");
    if (_list.size() != 0)
        throw InitializationFailureException(__func__, 
                                             "List size is not zero on initialization.");
}
catch (const InitializationFailureException& e) {
